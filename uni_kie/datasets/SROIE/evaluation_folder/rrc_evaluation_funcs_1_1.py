...
def get_tl_dict_values(
    ...
    if num_points < 3:
        raise Exception(
            "Incorrect format. Incorrect number of points. At least 3 points are necessary. Found: "
            + str(num_points)
        )

    if len(validNumPoints) > 0 and num_points in validNumPoints == False:
        raise Exception(
            "Incorrect format. Incorrect number of points. Only allowed 4,8 or 12 points)"
        )

    for i in range(num_points):
        if isinstance(detection["points"][i], list) == False:
            raise Exception(
                "Incorrect format. Point #" + str(i + 1) + " has to be an array)"
            )

        if len(detection["points"][i]) != 2:
            raise Exception(
                "Incorrect format. Point #"
                + str(i + 1)
                + " has to be an array with 2 objects(x,y) )"
            )
        ...
        if (
            ...
            raise Exception(
                "Incorrect format. Point #"
                + str(i + 1)
                + " childs have to be Integers)"
            )

        if imWidth > 0 and imHeight > 0:
            validate_point_inside_bounds(
                detection["points"][i][0], detection["points"][i][1], imWidth, imHeight
            )
    ...
    if withConfidence:
        ...
        if detection["confidence"] < 0 or detection["confidence"] > 1:
            raise Exception(
                "Incorrect format. Confidence key has to be a float between 0.0 and 1.0"
            )
