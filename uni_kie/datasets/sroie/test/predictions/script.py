...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    for key in gt:
        ...
        if not key in subm:
            raise Exception(
                ("The Det file is not valid (file %s.txt not found)" % (key)).encode(
                    "utf-8", "replace"
                )
            )
        ...
        if isinstance(gtJson, dict) == False:
            raise Exception(
                (
                    "The GT file is not valid (file %s.txt) The root element must be a dictionary."
                    % (key)
        ...
        for intKey in gtJson:
            if not intKey in ["company", "date", "address", "total"]:
                raise Exception(
                    (
                        "The GT file is not valid (file %s.txt) Key %s not valid (must be company,date,address or total)"
                        % (key, intKey)
                    ).encode("utf-8", "replace")
                )
        ...
        if isinstance(detJson, dict) == False:
            raise Exception(
                (
                    "The Det file is not valid (file %s.txt) The root element must be a dictionary."
                    % (key)
                ).encode("utf-8", "replace")
            )
        ...
        for intKey in detJson:
            if not intKey in ["company", "date", "address", "total"]:
                raise Exception(
                    (
                        "The Det file is not valid (file %s.txt) Key %s not valid (must be company,date,address or total)"
                        % (key, intKey)
                    ).encode("utf-8", "replace")
                )
            ...
            if isinstance(detJson[intKey], (dict, list, int, float)) == True:
                raise Exception(
                    (
                        "The Det file is not valid (file %s.txt) Key %s must be an string"
                        % (key, intKey)
                    ).encode("utf-8", "replace")
                )
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for key in gt:
        ...
        precision = (
            float(0)
            if detNumCategories == 0
            else float(precisionAccum) / detNumCategories
        )
        recall = (
            float(1) if gtNumCategories == 0 else float(recallAccum) / gtNumCategories
        )
        hmean = (
            0
